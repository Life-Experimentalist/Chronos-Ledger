...
- WebSockets: 500 authenticated sockets held open for a minute with no failures. At 2,000, nginx stopped at about 510, because its default `worker_connections 1024` counts both sides of each proxied socket. Raise it in `nginx/nginx.conf` if you expect more live dashboards than that.
...
| Reads | 100 | not sustainable | 68 req/s, p95 2.3 s |
...
- nginx `worker_connections 1024` caps live WebSockets at about 500 per proxy.
