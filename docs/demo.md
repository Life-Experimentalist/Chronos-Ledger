...
Signing in at **http://localhost** routes each person to their own portal
...
3. **Member**: sign in as `member@demo.internal`. Today's sessions are
   listed, and attendance can be marked. Demo sessions carry no location
   pin, so this works from any laptop; in a real deployment each session
   is geo-fenced to its room's coordinates.
4. **Guest kiosk** (`http://localhost/guest/kiosk/`): no login needed. A
   visitor registers a check-in here and the responsible staff member gets
   a live notification to approve or decline.
5. **API** (`http://localhost/docs`): the whole API, interactive, if your
...
Running the seed twice is safe; it does nothing the second time.
