...
- `DB_PASSWORD`, required, no default. Both compose files refuse to interpolate without it, so `docker compose up` stops before a container starts rather than falling back to a password this repository publishes.
