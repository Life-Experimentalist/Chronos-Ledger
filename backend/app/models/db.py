...
from sqlalchemy import (
    ...
    Enum, ForeignKey, Integer, Numeric, String, Text, Time, UniqueConstraint,
)
...
class AcademicCycle(Base):
    ...
    course_offerings = relationship("CourseOffering", back_populates="cycle", cascade="all, delete-orphan")
...
class User(Base):
    ...
    reporting_line_manager = Column(String(50), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class CourseOffering(Base):
    ...
    master_slots = relationship("StructuralMasterSlot", back_populates="course_offering", cascade="all, delete-orphan")
    registrations = relationship("CourseRegistration", back_populates="course_offering", cascade="all, delete-orphan")


class CourseRegistration(Base):
    __tablename__ = "course_registrations"
    __table_args__ = (UniqueConstraint("course_offering_id", "student_id", name="unique_student_registration"),)
...
class StructuralMasterSlot(Base):
    ...
    course_offering_id = Column(Integer, ForeignKey("course_offerings.id", ondelete="CASCADE"), nullable=False)
    primary_instructor_id = Column(String(50), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    ...
    daily_ledger_entries = relationship("DailyLedger", back_populates="master_slot", cascade="all, delete-orphan")
...
class DailyLedger(Base):
    ...
    master_slot_id = Column(Integer, ForeignKey("structural_master_slots.id", ondelete="CASCADE"), nullable=True)
    course_offering_id = Column(Integer, ForeignKey("course_offerings.id", ondelete="CASCADE"), nullable=False)
    ...
    verification_records = relationship("VerificationLedger", back_populates="ledger_instance", cascade="all, delete-orphan")
    annotations = relationship("LedgerAnnotation", back_populates="ledger_instance", cascade="all, delete-orphan")
...
class ReverseRsvpLog(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    submitting_user_id = Column(String(50), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    target_absence_date = Column(Date, nullable=False)
    context_justification = Column(Text, nullable=False)
    approval_state = Column(Enum(LogVerificationState), default=LogVerificationState.PENDING_VERIFICATION)
    authorized_by_user_id = Column(String(50), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class VerificationLedger(Base):
    ...
    __table_args__ = (UniqueConstraint("ledger_instance_id", "student_id", name="single_student_per_instance_record"),)

    id = Column(BigInteger, primary_key=True, index=True)
    ledger_instance_id = Column(BigInteger, ForeignKey("daily_ledger.id", ondelete="CASCADE"), nullable=False)
    student_id = Column(String(50), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    ...
    modification_timestamp = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
...
class LedgerAnnotation(Base):
    ...
    distribution_timestamp = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
...
class GuestGateRegistry(Base):
    ...
    target_faculty_id = Column(String(50), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    visitation_intent = Column(Text, nullable=False)
    handshake_status = Column(Enum(LogVerificationState), default=LogVerificationState.PENDING_VERIFICATION)
