      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search course, room..."
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-chronos-border/40">
                <th className="text-left px-4 py-3 text-xs font-semibold text-chronos-text-dim uppercase tracking-wider">Course</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-chronos-text-dim uppercase tracking-wider">Instructor</th>
                ...
                filtered.map((entry) => (
                  <tr key={entry.id} className="table-row">
                    <td className="px-4 py-3">
                      <p className="font-medium text-chronos-text">{entry.course_code}</p>
                      <p className="text-xs text-chronos-text-dim">{entry.course_title}</p>
                    </td>
                    ...
                    <td className="px-4 py-3 text-chronos-text-dim text-xs">
                      {entry.substitute_instructor_id
                        ? <><span className="text-chronos-warning">{entry.substitute_instructor_id}</span> <span className="text-chronos-muted">(proxy)</span></>
                        : entry.active_instructor_id || '—'}
                    </td>
